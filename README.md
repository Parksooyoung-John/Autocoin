...
| `MAX_OPEN_POSITIONS` | Demo 초반은 `1`, 안정화 후 `2`를 권장합니다. | 동시에 열 수 있는 포지션 수를 제한합니다. |
| `RISK_PER_TRADE_PERCENT` | Demo 초반은 `0.3`~`0.5`, 기본값은 `1.5`입니다. | 1회 거래에서 계좌 기준 감수할 최대 손실률입니다. ATR 손절폭과 함께 주문 수량을 계산합니다. |
| `MAX_DAILY_LOSS_PERCENT` | Demo 초반은 `2`, 기본값은 `5`입니다. | 하루 손실률이 이 값에 도달하면 신규 진입을 막습니다. |
...
TradingView는 내 PC의 `localhost`에 직접 접근할 수 없습니다. 로컬에서 연습하려면 ngrok 같은 HTTPS 터널이 필요합니다.

```powershell
ngrok http 8000
```

ngrok이 보여주는 HTTPS 주소에 `/webhook`을 붙여 TradingView에 입력합니다.

```text
https://xxxx.ngrok-free.app/webhook
```

효과: TradingView Alert가 인터넷을 통해 내 로컬 FastAPI 서버로 도착합니다. ngrok 무료 주소는 재실행할 때 바뀔 수 있으므로 TradingView Webhook URL도 같이 업데이트해야 합니다.
...
Demo 연습도 TradingView Alert 연동 기준으로 진행합니다.

1. `.env`에서 안전 설정을 낮게 시작합니다.
...
4. TradingView Alert는 BTCUSDT 1개만 먼저 연결합니다.
5. 첫날은 BTCUSDT만 운영합니다.
6. 정상 확인 후 ETHUSDT 추가, 이후 XRPUSDT 추가합니다.
7. 매일 아래 항목을 기록합니다.
...
효과: `pause`는 신규 진입만 막습니다. 이미 열린 포지션은 Binance에 등록된 손절/익절 주문 또는 수동 청산으로 관리해야 합니다.
...
Demo에서 확인해야 할 것은 "수익이 났는가"보다 먼저 "시스템이 위험한 행동을 하지 않는가"입니다.
...
- 실전 전 `BINANCE_TESTNET=true`로 최소 2주 Demo 운영
- `RISK_PER_TRADE_PERCENT=0.3` 이하로 시작
- `MAX_OPEN_POSITIONS=1`로 시작
- ngrok 대신 안정적인 HTTPS 배포 주소 권장
